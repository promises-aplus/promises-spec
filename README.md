...
1. Otherwise, if `x` is an object or function,
   ...
   1. If `then` is a function, call it with `x` as `this`, first argument `fulfillPromise`, and second argument `rejectPromise`, where:
      1. If/when `fulfillPromise` is called with a value `y`,
      ...
      1. If both `fulfillPromise` and `rejectPromise` are called, or multiple calls to the same argument are made, the first call takes precedence, and any further calls are ignored.
      1. If calling `then` throws an exception `e`,
         1. If `fulfillPromise` or `rejectPromise` have been called, ignore it.
